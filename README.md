...
> **Claude 사용량 전체가 집계됩니다.** Anthropic의 사용량 한도는 계정 단위로 통합돼 있어,
> 웹·데스크톱·모바일 Claude 앱과 Claude Code가 하나의 사용량 풀을 공유합니다.
> 따라서 채팅만 쓰더라도 그 사용량이 위젯 수치에 그대로 반영됩니다.
...
**Claude Code를 실제로 사용할 필요는 없습니다.** 위젯은 Claude Code가 로그인 시 저장해 두는
인증 정보(`~/.claude/.credentials.json`)를 읽어 사용량을 조회합니다. 그래서 처음 한 번은
[로그인 시작]이 필요하지만, 그 뒤로는 터미널을 열지 않고 채팅만 써도 수치가 정상적으로 갱신됩니다.
(브라우저의 claude.ai 로그인 세션만으로는 이 파일이 만들어지지 않아 위젯이 수치를 읽지 못합니다.)
...
설치 옵션을 고를 필요는 없습니다. 관리자 권한 없이 현재 사용자 폴더에 자동으로 설치되고, 끝나면 위젯이 바로 실행됩니다.
...
| 30% 초과 ~ 50% | 좌우로 빠르게 걷기 |
| 50% 초과 ~ 80% | 위아래로 뛰기 |
| 80% 초과 ~ 100% 미만 | 몸에 불이 붙어 사방으로 날뛰기 |
| 100% | 한도 소진. 회색으로 굳고 머리 위에 링이 생김 |

5시간 한도를 다 쓰면 캐릭터가 멈춥니다. 다음 충전 시각은 기본 창에서 확인할 수 있습니다.

커서를 올리면 우상단에 최소화 · 새로고침 · 설정 · 종료 버튼이 나타납니다. 캐릭터를 끌어서 위치를 옮길 수 있습니다.

다섯 구간의 움직임은 [character-motion-preview.html](character-motion-preview.html) 을 브라우저로 열면 한 화면에서 모두 볼 수 있습니다. 위젯과 동일한 CSS로 동작합니다.
